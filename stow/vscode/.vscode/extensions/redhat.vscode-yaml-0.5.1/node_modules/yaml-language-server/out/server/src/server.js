...
function getSchemaStoreMatchingSchemas() {
    return request_light_1.xhr({ url: JSON_SCHEMASTORE_URL }).then(response => {
...
function isKubernetes(textDocument) {
    for (const path in specificValidatorPaths) {
        ...
        const fpa = new jsonSchemaService_1.FilePatternAssociation(globPath);
    ...
    if (clientDynamicRegisterSupport) {
        ...
        if (enableFormatter) {
            if (!formatterRegistration) {
                formatterRegistration = connection.client.register(vscode_languageserver_1.DocumentRangeFormattingRequest.type, {
                    documentSelector: [
