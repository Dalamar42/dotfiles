...
class KubernetesResourceDefinitionProvider {
    ...
    provideDefinitionAsync(document, position) {
        return __awaiter(this, void 0, void 0, function* () {
            ...
            const parentLine = getParentLine(document, position);
            const siblingNSValue = getSiblingNamespace(document, position);
            const source = {
                key: currentLine.key,
                value: currentLine.value,
                parentKey: parentLine ? parentLine.key : null,
                siblingNamespaceValue: siblingNSValue,
...
function getParentLine(document, position) {
...
function getSiblingNamespace(document, position) {
    // Handles the k8s API ObjectReference structure
    const siblingNSLines = yaml_navigation_1.findSiblingsYaml(document, position.line)
        .map((i) => parseYamlLine(document, i))
        .filter((l) => l.key === "namespace");
    const siblingNSValue = siblingNSLines.length > 0 ? siblingNSLines[0].value : null;
    return siblingNSValue;
}
function findNavigationTarget(source) {
    ...
    switch (source.kind) {
        ...
        case kuberesources.allKinds.pod.abbreviation:
            return findNavigationTargetFromPod(source);
...
// TODO: pod and deployment are both really about PodTemplateSpec so are duplicate
// - can we extract?
function findNavigationTargetFromPod(source) {
    if (source.key === 'claimName' && source.parentKey === 'persistentVolumeClaim') {
        return kuberesources_virtualfs_1.kubefsUri(null, `pvc/${source.value}`, 'yaml');
    }
    else if (source.key === 'name' && source.parentKey === 'configMap') {
...
function findNavigationTargetFromPV(source) {
    if (source.key === 'storageClassName') {
        ...
        return kuberesources_virtualfs_1.kubefsUri(source.siblingNamespaceValue, `pvc/${source.value}`, 'yaml');
    }
    else {
        return undefined;
    }
