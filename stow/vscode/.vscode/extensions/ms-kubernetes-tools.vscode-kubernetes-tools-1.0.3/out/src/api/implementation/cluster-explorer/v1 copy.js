...
function resourceFolderContributor(displayName, pluralDisplayName, manifestKind, abbreviation) {
    const nodeSource = new extension_nodesources_1.CustomResourceFolderNodeSource(new kuberesources_1.ResourceKind(displayName, pluralDisplayName, manifestKind, abbreviation));
...
function resourcesNodeSource(manifestKind, abbreviation, options) {
    ...
    const nodeSource = new extension_nodesources_1.ResourcesNodeSource(new kuberesources_1.ResourceKind(manifestKind, manifestKind, manifestKind, abbreviation), optionsImpl);
    return apiNodeSourceOf(nodeSource);
}
function adaptResourcesNodeSourceOptions(source) {
    ...
    return {
        lister: lister,
        filter: filter ? (cern) => filter(adaptKubernetesExplorerResourceNode(cern)) : undefined,
...
//     };
// }
//# sourceMappingURL=v1 copy.js.map